...
const consumerAddress = "0x852116945835086732047bd325Aa5Bc8C94C5305"; // Asset value updater sol
const subscriptionId = 88; // REPLACE this with your subscription ID

const makeRequestSepolia = async () => {
  // hardcoded for Ethereum Sepolia
  const routerAddress = "0xb83E47C2bC239B3bf370bc41e1459A34b41238D  	0";
  const linkTokenAddress = "0x779877A7B0D9E8603169DdbD7836e478b4624789";
  const donId = "fun-ethereum-sepolia-1";
  const explorerUrl = "https://sepolia.etherscan.io";

  // Initialize functions settings
  const source = fs.readFileSync(path.resolve(__dirname, "./source/getPrices.js")).toString();

  const propertyOwner = "";
  const propertyId = "1";
  const args = [propertyOwner, propertyId]; // args are not important for simulation. This just needs to be passed back to fulfill requests onchain
  ...
  (async () => {
    try {
      ...
      if (fulfillmentCode === FulfillmentCode.FULFILLED) {
        console.log(
          `\n✅ Request ${response.requestId} successfully fulfilled. Cost is ${ethers.utils.formatEther(
      ...
      if (errorString) {
        ...
        const responseBytesHexstring = response.responseBytesHexstring;
        if (ethers.utils.arrayify(responseBytesHexstring).length > 0) {
          const decodedResponse = decodeResult(response.responseBytesHexstring, ReturnType.bytes);
          console.log(`\n✅ Decoded response to ${ReturnType.bytes}: `, decodedResponse);
        }
      }
    } catch (error) {
